...
bias_valid = pd.read_csv('results/TFIDF/tfidf_bias_train_custom.csv')
...
def predict_bias(new_text, model, word_list, lda_results):
    ...
    # Make prediction (output probabilities)
    prediction = model.predict(combined_features)
    
    # Get the probability for the "bias" class (assume it's the second class, index 1)
    bias_class_index = 1  # Assuming 'bias' is the second class (index 1)
    bias_probability = prediction[0][bias_class_index]  # Probability of being biased

    # Get predicted class
    predicted_class = le.inverse_transform(np.argmax(prediction, axis=1))[0]

    # Return both predicted class and bias probability as percentage
    return predicted_class, bias_probability * 100  # Bias as a percentage

# Example usage
new_article = "h."
predicted_class, bias_percentage = predict_bias(new_article, model, word_list, lda_results)
print(f"Predicted bias: {predicted_class} with {bias_percentage:.2f}% bias.")
