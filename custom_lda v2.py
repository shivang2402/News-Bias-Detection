...
bias_data = pd.read_csv('../News-Bias-Detection/TFIDF/tfidf_bias_train_custom.csv')  # CSV with TF-IDF values for the bias data
aspects_data = pd.read_csv('../News-Bias-Detection/TFIDF/tfidf_aspects_custom.csv')  # CSV with TF-IDF values for the aspects data
...
# Custom LDA class
class CustomLDA:
    def __init__(self, corpus, num_topics=5, alpha=0.01, beta=0.01, num_iterations=100):
...
def expand_keywords_with_similarity(top_keywords, corpus, top_n=5):
...
def map_topics_to_aspects(lda, aspects_data, top_n_words=15):
    ...
    for topic_id in range(lda.num_topics):
        ...
        expanded_keywords = expand_keywords_with_similarity(top_words, aspects_corpus, top_n=5)
        aspect_tfidf_matrix = []
...
# Train the LDA model
lda = CustomLDA(bias_corpus, num_topics=15, num_iterations=50)
lda.train()
